...
### Positioning
- README now opens with **"What polylens is"** — a small-model interpretability
...
- `polylens bench MODEL --out report.md` now writes human-readable markdown.
...
- Wrapped `TransformerBackend.extract` and `attribute.activation_patch`
  ...
  graphs.
- `cli.py`: lazy-imported torch/transformers so `polylens info` is fast.

### Renamed (internal)
- The library was renamed from `mechinterp-small` to `polylens` between v0.1
...
### New modules
- **`polylens.lens`** — Logit lens + Tuned lens (Belrose et al 2023).
  ...
  - `TunedLens.fit(model, tokenizer, calibration_texts)` — learned per-layer
    affine translations
- **`polylens.diff`** — Compare base vs fine-tuned models. Returns per-layer
